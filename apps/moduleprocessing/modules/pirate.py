...
class PirateDataCrawler:
    def __init__(self):
        # 크롬 드라이버 경로 설정
        self.__chrome_driver_path = chrome_driver_path = os.path.join(settings.STATIC_ROOT, 'chromedriver')
    ...
    def getDriver(self):return webdriver.Chrome(self.__chrome_driver_path, options=self.__options)
    def getPirateData(self,countryName):
        # 드라이버로 페이지 열기
        url = 'https://www.gicoms.go.kr/pirate/pirate_07.do'
        try:
            driver = webdriver.Chrome(self.__chrome_driver_path, options=self.__options)
            driver.get(url)
            # "구역" select 태그 선택
